...
- `download_pdf` now validates the URL scheme and resolved host (refuses
  private, loopback, link-local, CGNAT, and ULA networks) and verifies the
  downloaded bytes start with `%PDF-`.
- `download_pdf` now writes to `<name>.pdf.part` and renames atomically on
  success — a failed or oversized download never leaves a partial file at
  the final path.
- `search_papers` now uses OpenAlex's top-level `?search=` parameter instead
  of the deprecated `filter=fulltext.search:` form, restoring correct
  relevance scoring and removing comma-injection risk.
...
- `Paper.funder` (singular) renamed to `Paper.funders` (it's a list). Funder
  lists from OpenAlex and CrossRef are now merged and de-duplicated.
- Tool errors are now surfaced as structured errors with sanitized messages
  rather than raw upstream response bodies.

### Fixed

- Path traversal via `download_pdf(filename=...)` — caller-supplied filenames
  are now sanitized and the resolved destination is checked to stay inside
  `PDF_DIR`.
...
- 26 offline unit tests via `respx`.
